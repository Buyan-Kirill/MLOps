...
1) Настройка окружения и данных:
...
pip install -r requirements.txt (может грузиться довольно долго)
...
2) Запуск эксперимента и логирование:

Можно запустить пайплайн заново, для этого нужно выполнить следующие шаги:

mkdir -p logs/ processed_data/ outputs/

dvc repro (в таком случае в папке mlruns появится информация о текущем запуске; время одного запуска около часа)

3) Оффлайн-инференс:

docker build -t ml-app:v1 .

(Образ получается более 1 ГБ (где-то 2 ГБ), но это из-за наличия тяжёлых библиотек по типу torh которые суммарно занимают порядка 1.5 ГБ)
