...
def main(config_path: str) -> None:
    ...
    books = pd.read_parquet(os.path.join(INPUT_DIR, "books_clean.parquet"))
    ...
    books = (
        books[books["title"].notna() & (books["title"] != "")]
        .copy()
    ...
    series_emb = model.encode(books["series_clean"].tolist(), show_progress_bar=True)
    logger.info("Кодирование авторов")
    author_emb = model.encode(books["author_clean"].tolist(), show_progress_bar=True)
    logger.info("Кодирование описаний")
    desc_emb = model.encode(books["description_clean"].tolist(), show_progress_bar=True)
